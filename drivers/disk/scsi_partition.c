...
static int scsi_part_mbr_fat_partition(const uint8_t *mbr, uint32_t *base_lba, uint32_t *part_sectors)
{
					       ...
					       uint64_t disk_sectors)
{
	...
	if (volume_base + part_sectors < volume_base ||
	    volume_base + part_sectors > disk_sectors) {
		if (volume_base >= disk_sectors) {
			return 0U;
		}
		return disk_sectors - volume_base;
					...
					uint32_t *part_sectors)
{
	...
	for (uint32_t sec = 0U; sec < 4U; sec++) {
		...
		for (uint32_t i = 0U; i < entries_per_sec && (sec * entries_per_sec + i) < part_count;
		     i++) {
