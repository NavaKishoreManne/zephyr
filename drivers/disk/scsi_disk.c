...
static int scsi_disk_ensure_bounce(struct scsi_disk *ctx)
{
	...
	ctx->bounce_buf = k_aligned_alloc(CONFIG_SCSI_DISK_BUF_ALIGN,
					  CONFIG_SCSI_DISK_BOUNCE_SIZE);
	if (ctx->bounce_buf == NULL) {
		return -ENOMEM;
	}
...
static int scsi_disk_xfer_blocks(struct scsi_disk *ctx, uint64_t lba, uint32_t blocks,
				 void *buf, bool write)
{
#if IS_ENABLED(CONFIG_SCSI_DISK_BOUNCE_BUF)
	const uint32_t block_size = ctx->sdev->block_size;
...
static int scsi_disk_access_read(struct disk_info *disk, uint8_t *data_buf,
