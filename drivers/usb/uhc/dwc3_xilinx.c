...
static inline mm_reg_t dwc3_xilinx_usb2_base(const struct dwc3_xilinx_config *cfg)
{
	return (mm_reg_t)cfg->usb2_wrapper_base;
}

static inline uint32_t dwc3_xilinx_usb2_readl(const struct dwc3_xilinx_config *cfg,
					      uint32_t offset)
{
	return sys_read32(dwc3_xilinx_usb2_base(cfg) + offset);
}

static inline void dwc3_xilinx_usb2_writel(const struct dwc3_xilinx_config *cfg,
					   uint32_t offset, uint32_t val)
{
	sys_write32(val, dwc3_xilinx_usb2_base(cfg) + offset);
}

static void dwc3_xilinx_usb2_phy_rst_mask(const struct dwc3_xilinx_config *cfg, bool mask)
{
	uint32_t reg = dwc3_xilinx_usb2_readl(cfg, VERSAL_USB2_PHY_RST);
	...
	dwc3_xilinx_usb2_writel(cfg, VERSAL_USB2_PHY_RST, reg);
}
...
static void dwc3_xilinx_set_coherency_route(const struct dwc3_xilinx_config *cfg)
{
	uint32_t reg = dwc3_xilinx_usb2_readl(cfg, VERSAL_USB2_COHERENCY);
	...
	dwc3_xilinx_usb2_writel(cfg, VERSAL_USB2_COHERENCY, reg);
}

static void dwc3_xilinx_apply_jitter_adjust(const struct dwc3_xilinx_config *cfg,
					    mm_reg_t dwc3_base)
{
	uint32_t gfl = dwc3_readl(dwc3_base, DWC3_GFLADJ) & DWC3_GFLADJ_30MHZ_MASK;
	uint32_t jit = dwc3_xilinx_usb2_readl(cfg, VERSAL_USB2_JITTER_ADJUST);
	uint32_t new_jit;

	new_jit = (jit & ~VERSAL_USB2_JITTER_FLADJ_MASK) |
		  (gfl & VERSAL_USB2_JITTER_FLADJ_MASK);

	if (new_jit == jit) {
		return;
	}

	dwc3_xilinx_usb2_writel(cfg, VERSAL_USB2_JITTER_ADJUST, new_jit);
}

void dwc3_xilinx_pre_host_burst(const struct dwc3_xilinx_config *cfg)
{
	if (!dwc3_xilinx_wrapper_present(cfg)) {
		return;
	}

	dwc3_xilinx_usb2_phy_rst_mask(cfg, false);
}
...
void dwc3_xilinx_host_tune_post(const struct dwc3_xilinx_config *cfg, mm_reg_t dwc3_base)
{
	...
	dwc3_xilinx_usb2_phy_rst_mask(cfg, true);
	dwc3_xilinx_apply_gfladj(dwc3_base, fladj30mhz);
	dwc3_xilinx_apply_jitter_adjust(cfg, dwc3_base);
	dwc3_xilinx_apply_guctl1_host(dwc3_base);
	dwc3_xilinx_clear_susphy(cfg, dwc3_base);
	dwc3_xilinx_set_coherency_route(cfg);

	UHC_DWC3_DBG("xilinx host tune done wrapper=0x%08x",
		(unsigned int)cfg->usb2_wrapper_base);
}
