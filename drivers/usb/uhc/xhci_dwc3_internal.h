...
static inline void xhci_writeq(const struct uhc_dwc3_data *priv, mm_reg_t base,
...
static inline struct xhci_ep_ctx *xhci_slot_output_ep_ctx(
	const struct uhc_dwc3_data *priv, unsigned int dci)
{
...
static inline void ring_ep_doorbell(struct uhc_dwc3_data *priv, uint32_t slot_id,
				    uint32_t ep_id)
{
...
void dwc3_build_port_poll_order(unsigned int *order, unsigned int *n_order,
				unsigned int maxp, unsigned int usb2_first);
...
void xhci_poll_boot_connected_device(struct uhc_dwc3_data *priv,
...
/* xhci_ep0.c */
void xhci_flush_ep0_td(struct xhci_ring *ring, struct xhci_trb *setup,
		       struct xhci_trb *data, struct xhci_trb *status);
...
void xhci_ep0_verify_mps_matches(struct uhc_dwc3_data *priv, uint16_t expect,
				 const char *tag);
...
int dwc3_xfer_sem_take_ep0(const struct device *dev, struct uhc_dwc3_data *priv,
			   k_timeout_t timeout);
int xhci_address_device_initial(struct uhc_dwc3_data *priv, uint8_t port,
				uint8_t speed);
