...
{
	uint64_t trb_phys = xhci_bulk_dma_addr(ring->trbs) +
			    (uint64_t)ring->dequeue * 16ULL;
...
void xhci_bulk_td_completed(const struct xhci_bulk_hcd_ops *ops, uint8_t dci,
			    struct xhci_ring *ring, const struct xhci_td *td,
			    bool bulk_in, int br)
{
	ARG_UNUSED(bulk_in);
...
int xhci_bulk_recover_ep(const struct xhci_bulk_hcd_ops *ops, uint8_t dci,
			 struct xhci_ring *ring)
{
	...
	if (ops == NULL || ring == NULL || ops->ctx == NULL || ops->slot_id == 0U ||
	    dci < 2U || dci >= 32U || ring->trbs == NULL) {
		return -EINVAL;
	}
			     ...
			     struct xhci_ring *ring, bool bulk_in)
{
	...
	if (ops == NULL || ring == NULL || ring->trbs == NULL || ops->slot_id == 0U ||
	    dci < 2U || dci >= 32U) {
		return -EINVAL;
	}
	...
	if (ops->sync_ring_from_hw != NULL) {
		ops->sync_ring_from_hw(ops->ctx, dci);
		LOG_DBG("xhci_bulk: DCI%u queue prologue align deq=%u cyc=%u",
			(unsigned int)dci, (unsigned int)ring->dequeue,
...
bool xhci_bulk_event_is_interior(const struct xhci_td *td, uint64_t evt_trb_ptr,
				 uint32_t cc, uint64_t ioc_trb_phys)
{
