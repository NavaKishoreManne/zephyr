 ...
 */
int xhci_send_command_ex(struct uhc_dwc3_data *priv, uint32_t param_lo,
				uint32_t param_hi, uint32_t status, uint32_t control,
				bool drain_evt_ring)
{
	unsigned int pcs;
...
int xhci_send_command(struct uhc_dwc3_data *priv, uint32_t param_lo,
			     uint32_t param_hi, uint32_t status, uint32_t control)
{
...
int xhci_cmd_configure_endpoint(struct uhc_dwc3_data *priv)
{
	...
	return xhci_send_command(priv, (uint32_t)inp_phys, (uint32_t)(inp_phys >> 32),
				 0, control);
}
...
struct usb_ep_descriptor *xhci_ep_desc_for_dci(struct usb_device *udev,
						      uint8_t dci)
{
	unsigned int n;
	bool in;
...
uint32_t xhci_int_ep_info_field(const struct usb_ep_descriptor *epd,
...
static void xhci_inf_dump_slot_out(const struct uhc_dwc3_data *priv, const char *tag)
{
	...
	UHC_DWC3_DBG("[%s] slot OUT dev_info=0x%08x LAST_CTX=%u usb_addr=0x%02x",
		tag, di, (unsigned int)last_ctx, (unsigned int)(s->dev_state & 0xffU));
}
 ...
 */
int xhci_dwc3_bulk_output_eps_steady(struct uhc_dwc3_data *priv,
 ...
 */
void xhci_dwc3_verify_post_configure(struct uhc_dwc3_data *priv,
					    struct usb_device *udev,
					    ...
					    unsigned int max_dci)
{
	...
	UHC_DWC3_DBG("bulk need IN=%u OUT=%u; OutputDevCtx RUNNING+deq IN=%u OUT=%u",
		bulk_in_need, bulk_out_need, bulk_in_running, bulk_out_running);
	if (bulk_in_need > 0U && !bulk_in_running) {
					     ...
					     const uint8_t *dci_has_desc)
{
	...
	for (unsigned int dci = 0U; dci <= max_dci; dci++) {
		memcpy(inp + slot_off + (size_t)dci * (size_t)cb,
		       out + (size_t)dci * (size_t)cb, (size_t)cb);
	}
 ...
 */
int xhci_dwc3_configure_non_ep0(struct uhc_dwc3_data *priv,
				       struct usb_device *udev)
{
	...
	for (unsigned int dci = 2U; dci <= max_dci; dci++) {
...

		{
			...
			dwc3_dma_flush(priv->ep_bulk_trbs[dci],
				       sizeof(priv->ep_bulk_trbs[dci]));
			...
			if (type == USB_EP_TYPE_BULK) {
				 ...
				 * request RUNNING; ep_info=0 leaves Invalid on some DWC3+xHCI IP and
				 * the HC may complete the command but never service bulk rings.
				 */
				ep->ep_info = XHCI_EP_CTX_EP_STATE_RUNNING;
				ep->ep_info2 =
					XHCI_EP_CTX_TYPE(dir_in ?
							 XHCI_EP_CTX_TYPE_BULK_IN :
							 XHCI_EP_CTX_TYPE_BULK_OUT) |
					XHCI_EP_CTX_CERR(3) |
					XHCI_EP_CTX_MAX_BURST(0) |
	...
	UHC_DWC3_DBG("Configure EP: issuing xHCI Configure Endpoint (slot=%u LAST_CTX=%u add_flags=0x%08x)",
	 ...
	 */
	{
		...
		if (ctx_mirror) {
			for (unsigned int dci = 2U; dci <= max_dci; dci++) {
				...
				if (out_ep->ep_info == 0U && out_ep->ep_info2 == 0U &&
					...
					mirrored++;
					LOG_WRN("Configure EP: output DCI%u zero after COMP_SUCCESS — mirrored input "
						"context (xHCI quirk)",
						(unsigned int)dci);
				}
			}
		...
		if (mirrored > 0U) {
			UHC_DWC3_DBG("Configure EP: OutputDevCtx fixup — mirrored %u endpoint context(s) "
				"(HC skipped writeback; addr=%u)",
				mirrored, (unsigned int)udev->addr);
		} else {
			UHC_DWC3_DBG("Configure EP: OutputDevCtx populated by HC (no mirror; addr=%u)",
	...
	UHC_DWC3_DBG("Configure EP: xHCI COMP_SUCCESS (LAST_CTX idx=%u) — starting OutputDevCtx verify",
		max_dci);
	xhci_dwc3_verify_post_configure(priv, udev, dci_has_desc, max_dci);
	priv->steady_after_configure_ep = true;
	UHC_DWC3_DBG("Configure EP: steady_after_configure_ep=true (expect no bus_reset until disconnect; "
		"addr=%u)",
		(unsigned int)udev->addr);
	return 0;
}
 ...
 */
int xhci_cmd_set_tr_dequeue_deq(struct uhc_dwc3_data *priv, uint32_t ep_index,
				       uint64_t deq)
{
	...
	return xhci_send_command_ex(priv, (uint32_t)deq, (uint32_t)(deq >> 32), 0, ctrl,
				    false);
}
 ...
 */
int xhci_bulk_eps_reconfigure_drop_add(struct uhc_dwc3_data *priv,
					      struct usb_device *udev,
					      bool force_drop_add)
{
	uint8_t *inp = priv->input_ctx;
	...
	UHC_DWC3_DBG("bulk refresh scan max_dci=%u udev_speed=%u addr=%u",
		(unsigned int)max_dci, (unsigned int)udev->speed, (unsigned int)udev->addr);
	...
	for (unsigned int i = 0U; i <= max_dci; i++) {
		memcpy(inp + slot_off + (size_t)i * (size_t)cb,
		       priv->dev_ctx + (size_t)i * (size_t)cb,
		       (size_t)cb);
	}
	...
	for (unsigned int dci = 2U; dci <= max_dci; dci++) {
		...
		need_fixup = (out_ep->ep_info == 0U && out_ep->ep_info2 == 0U &&
			      out_ep->deq == 0ULL) ||
...
int xhci_enable_slot(struct uhc_dwc3_data *priv)
{
	...
	ret = xhci_send_command(priv, 0, 0, 0,
...
void xhci_teardown_active_slot(struct uhc_dwc3_data *priv)
{
	...
	if (priv->ep0_active_xfer != NULL) {
		(void)xhci_cancel_ep_xfer(priv, XHCI_DCI_DEFAULT_CONTROL,
					  priv->ep0_active_xfer, -ECONNRESET);
	}

	for (unsigned int dci = 2U; dci < 32U; dci++) {
		if (priv->bulk_active_xfer[dci] != NULL) {
			(void)xhci_cancel_ep_xfer(priv, (uint8_t)dci,
						  priv->bulk_active_xfer[dci],
						  -ECONNRESET);
		}
	}

	LOG_WRN("Disable Slot %u (disconnect / reconnect)",
		(unsigned int)sid);
	ret = xhci_disable_slot_cmd(priv, sid);
	if (ret != 0) {
		LOG_WRN("Disable Slot %u failed (%d); clearing DCBAA + SW rings",
			(unsigned int)sid, ret);
	}
