...
void xhci_dwc3_bulk_sync_ring_from_hw(struct uhc_dwc3_data *priv, uint8_t dci)
{
	...
	if (ring->enqueue != idx || ring->dequeue != idx ||
	    (ring->cycle_state & 1U) != dcs) {
...
int xhci_dwc3_bulk_abort_td(struct uhc_dwc3_data *priv, uint8_t dci)
{
	...
	if (ret != 0) {
		LOG_WRN("abort DCI%u Set TR Dequeue failed: %d",
...
void xhci_dwc3_bulk_td_giveback(struct uhc_dwc3_data *priv, uint8_t dci, bool dir_in,
				int br, uint32_t req_len)
{
	struct xhci_ring *ring = uhc_dwc3_ep_bulk_ring(priv, dci);
