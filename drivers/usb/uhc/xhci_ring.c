		    ...
		    uint32_t link_extra_ctl)
{
	...
	trbs[last].status = 0;
	trbs[last].control = XHCI_TRB_TYPE(XHCI_TRB_LINK) | XHCI_TRB_LINK_TC |
			     XHCI_TRB_CYCLE | link_extra_ctl;
}

void xhci_ring_event_segment_init(struct xhci_ring *ring, struct xhci_trb *trbs,
				  uint32_t num_trbs)
{
...
{
	if (ring->trbs == NULL || num_trbs_needed == 0U ||
	    num_trbs_needed > ring->num_trbs - 2U) {
		return -ENOSPC;
	}
