...
void xhci_copy_ep0_dequeue_into_input_ctx(struct uhc_dwc3_data *priv)
{
	...
	ep0_in->deq = xhci_tr_deq_ptr(seg + (uint64_t)ring->enqueue * 16ULL,
				       ring->cycle_state & 1U);
}
...
bool xhci_ep0_ring_verify_dequeue_matches_sw(struct uhc_dwc3_data *priv)
{
	...
	if (idx == ring->num_trbs - 1U) {
		...
		UHC_DWC3_DBG("EP0 ring check: deq OOB deq=0x%llx",
			(unsigned long long)deq);
		return false;
	}
	...
	UHC_DWC3_DBG("EP0 ring OK before TD: HW/SW enq=%u cyc=%u",
		(unsigned int)idx, dcs);
	return true;
}
uint32_t ep0_td_size_packets_after_setup(uint16_t w_length, uint16_t mps0)
{
	...
	data_pkts = (uint32_t)(((uint32_t)w_length + (uint32_t)mps0 - 1U) /
			       (uint32_t)mps0);
	rem = data_pkts + 1U; /* data packet(s) + status */

	rem = (rem > 31U) ? 31U : rem;
	UHC_DWC3_DBG("TD_SIZE wLength=%u mps0=%u data_pkts=%u rem=%u",
		w_length, mps0, data_pkts, rem);
				       ...
				       uint8_t speed)
{
	...
	ep0->ep_info2 = XHCI_EP_CTX_TYPE(XHCI_EP_CTX_TYPE_CTRL) |
			XHCI_EP_CTX_CERR(3) |
			XHCI_EP_CTX_MAX_PACKET(mps);
	{
		...
		UHC_DWC3_DBG("EP0 ctx (input): deq=0x%llx DCS=%u seg=%llx",
			(unsigned long long)ep0->deq, dcs,
			(unsigned long long)ep0_seg);
	}
...

	{
		...
		ad_ret = xhci_send_command(priv, (uint32_t)inp_phys,
					   (uint32_t)(inp_phys >> 32), 0, control);
		if (ad_ret == 0) {
...
#if IS_ENABLED(CONFIG_UHC_DWC3_DEBUG)
			{
				struct xhci_ep_ctx *ep0_out =
					(struct xhci_ep_ctx *)(priv->dev_ctx +
							       priv->ctx_bytes);
...
int xhci_address_device_set_address_bsr0(struct uhc_dwc3_data *priv)
{
	...
	UHC_DWC3_DBG("AD(BSR=0) input: add=0x%08x drop=0x%08x | in_slot "
		...
		(unsigned int)priv->ep0_ring.enqueue,
		(unsigned int)priv->ep0_ring.dequeue,
...

	{
		...
		ret = xhci_send_command(priv, (uint32_t)inp_phys,
					(uint32_t)(inp_phys >> 32), 0, control);
	}
	...
	if (ret != 0) {
		if (ret == -ETIMEDOUT) {
			...
			LOG_WRN("AD(BSR=0) fail: ret=%d cmd_comp_code=%u",
				ret, (unsigned int)priv->cmd_comp_code);
		}
		return ret;
	}
	...
	UHC_DWC3_DBG("AD(BSR=0) post OK: out dev_state=0x%08x "
		...
		slot_out->dev_state,
		(unsigned int)(slot_out->dev_state & 0xffU),
		ep0_out->ep_info, ep0_out->ep_info2,
		(unsigned long long)ep0_out->deq);
