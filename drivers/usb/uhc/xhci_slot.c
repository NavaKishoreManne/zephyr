...
void xhci_teardown_slot(struct uhc_dwc3_data *priv, uint8_t sid)
{
	...
	if (slot->ep0_active_xfer != NULL) {
		(void)xhci_cancel_ep_xfer(priv, slot, XHCI_DCI_DEFAULT_CONTROL, slot->ep0_active_xfer,
					  -ECONNRESET);
	}
				     ...
				     struct usb_device *udev, uint8_t port, uint8_t speed)
{
	...
	dev_info = XHCI_SLOT_LAST_CTX(1) | XHCI_SLOT_SPEED(speed) | XHCI_SLOT_ROUTE(xhci_device_route(udev));

	if (udev->dev_desc.bDeviceClass == USB_BCC_HUB ||
	    (udev->dev_desc.bDeviceClass == 0U && udev->parent == NULL)) {
		/* Hub class may not have dev_desc yet on first address — allow hub interface later. */
	}
