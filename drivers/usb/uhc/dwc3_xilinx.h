...
struct dwc3_xilinx_config {
	/** USB2 wrapper MMIO base; 0 when the SoC has no wrapper block. */
	uintptr_t usb2_wrapper_base;
...
static inline bool dwc3_xilinx_wrapper_present(const struct dwc3_xilinx_config *cfg)
{
	return cfg != NULL && cfg->usb2_wrapper_base != 0U;
}
...
#define _DWC3_PLAT_WRAPPER_BASE(n) \
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, amd_usb2_wrapper_base), \
		    (DT_INST_PROP(n, amd_usb2_wrapper_base)), \
		    (COND_CODE_1(DT_INST_NODE_HAS_PROP(n, xlnx_usb2_wrapper_base), \
				 (DT_INST_PROP(n, xlnx_usb2_wrapper_base)), (0U))))

#define _DWC3_WRAPPER_OR_DEFAULT(n, val) \
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, amd_usb2_wrapper_base), (val), \
		    (COND_CODE_1(DT_INST_NODE_HAS_PROP(n, xlnx_usb2_wrapper_base), (val), (0U))))
...
#define _DWC3_DWC3_QUIRKS(n) \
	(_DWC3_QUIRK_FROM_DT_OR_INTEGRATED(n, snps_dis_u2_susphy_quirk, \
	 ...
	 _DWC3_QUIRK_FROM_DT_OR_INTEGRATED(n, snps_dis_u3_susphy_quirk, \
					    DWC3_QUIRK_DIS_U3_SUSPHY))
...
#define DWC3_XILINX_CONFIG_INIT(n) \
	{ \
		.usb2_wrapper_base = _DWC3_PLAT_WRAPPER_BASE(n), \
