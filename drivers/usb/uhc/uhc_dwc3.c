...
static void xhci_bulk_giveback_first_trb(struct uhc_dwc3_data *priv,
					 struct xhci_ring *ring,
					 struct xhci_trb *start_trb,
					 unsigned int start_cycle, uint8_t trb_count,
					 uint32_t slot_id, uint32_t dci)
{
...
static int dwc3_ep_sync_after_clear_feature_cb(const struct device *dev,
...
static int uhc_dwc3_init(const struct device *dev)
{
	...
	ret = xhci_reset(priv, cfg->xhci_base);
	if (ret != 0) {
...
static int uhc_dwc3_enable(const struct device *dev)
{
	...
	dwc3_host_susphy_post_run(cfg, false);
...
static int uhc_dwc3_disable(const struct device *dev)
{
	...
	timeout = 1000;
	while (!(xhci_readl(priv->op_base, XHCI_OP_USBSTS) & XHCI_USBSTS_HCH) &&
	       timeout > 0) {
		k_busy_wait(100);
		timeout--;
	}
...
static int uhc_dwc3_bus_reset(const struct device *dev)
{
	...
	uint32_t port_off = XHCI_OP_PORTSC_BASE +
			    ((uint32_t)connect_rp - 1U) * XHCI_PORT_REGS_STRIDE;
	uint32_t portsc;
	int timeout;
	int ret;
	...
	UHC_DWC3_DBG("bus_reset: enter CSC port %u HC N_PORTS=%u %d",
		(unsigned int)connect_rp, (unsigned int)priv->max_ports, port_off);
	...
	if (priv->max_ports >= 2U && priv->port_speed < XHCI_SPEED_SUPER) {
		...
		port_off = XHCI_OP_PORTSC_BASE +
			   ((uint32_t)reset_rp - 1U) * XHCI_PORT_REGS_STRIDE;
	...
	if (priv->port_speed < XHCI_SPEED_SUPER) {
		...
		UHC_DWC3_DBG("bus_reset: after recovery delay PORTSC=0x%08x CCS=%u PED=%u",
			portsc,
		...
		if (portsc & XHCI_PORTSC_PED) {
			...
			if (!(portsc & XHCI_PORTSC_PED)) {
				LOG_ERR("USB2 PED not set after reset (PORTSC=0x%08x)",
					portsc);
				return -EBUSY;
			}
		}
		...
		if (!(portsc & XHCI_PORTSC_PED)) {
			LOG_ERR("SS PED not set after warm reset (PORTSC=0x%08x)",
				portsc);
			return -EBUSY;
		}
	}
...
static int uhc_dwc3_assign_address(const struct device *dev,
				   struct usb_device *udev,
				   uint8_t *addr_out)
{
	struct uhc_dwc3_data *priv = uhc_get_private(dev);
	...
	hc_addr = (uint8_t)(((const struct xhci_slot_ctx *)priv->dev_ctx)->dev_state &
			    0xffU);
	if (hc_addr == 0U) {
	...
	if (USB_EP_GET_IDX(xfer->ep) == 0U) {
		...
		trb_setup->status = XHCI_TRB_LEN(8) | XHCI_TRB_INTR_TARGET(0);
		{
			...
			UHC_DWC3_DBG("ep0 ctrl ep=0x%02x type=0x%02x req=0x%02x wVal=0x%04x "
				...
				sys_le16_to_cpu(setup->wValue), wLength,
				priv->ep0_max_packet, mps, td_sz,
		...
		uint32_t status_dir = has_data ? (dir_in ? 0U : XHCI_TRB_DIR_IN)
					       : XHCI_TRB_DIR_IN;

		trb_status->control = XHCI_TRB_TYPE(XHCI_TRB_STATUS) |
				      XHCI_TRB_IOC | status_dir |
		...
		UHC_DWC3_DBG("EP submit: ep=0x%02x ep_id=%u ring=ep0",
			xfer->ep, (unsigned int)XHCI_DCI_DEFAULT_CONTROL);
		/*
		...
		if (dwc3_xfer_sem_take_ep0(dev, priv, K_MSEC(5000)) != 0) {
			...
			LOG_ERR("EP0 timeout diag: ctx deq=0x%016llx ep_info=0x%08x "
				"STATE=%u | sw_enq=%u sw_cyc=%u link_CHAIN=%u | slot=%u doorbell_dci=%u",
				...
				(unsigned int)((ring->trbs[ring->num_trbs - 1U].control & XHCI_TRB_CHAIN) !=
					       0U),
				(unsigned int)priv->slot_id,
				(unsigned int)XHCI_DCI_DEFAULT_CONTROL);
			LOG_ERR("EP0 timeout: ep_info2=0x%08x tx_info=0x%08x",
				ep0_dbg->ep_info2, ep0_dbg->tx_info);
			...
			if (xfer->buf && dir_in && priv->xfer_result == 0) {
				...
				uint32_t got = xhci_in_bytes_from_event(
					wLength, lenfield, priv->xfer_comp_code);
		...
		if (xfer->err == 0) {
			UHC_DWC3_DBG("EP0 xfer OK COMP=%u lenfield=%u",
				priv->xfer_comp_code,
		...
		if (xfer->err == 0 && has_data && dir_in && xfer->buf != NULL &&
		    ...
		    USB_REQTYPE_GET_TYPE(setup->bmRequestType) ==
			...
			if (priv->port_speed == XHCI_SPEED_FULL && mps0 != 64U &&
			    mps0 != 0U &&
			    (mps0 == 8U || mps0 == 16U || mps0 == 32U)) {
				(void)xhci_evaluate_ep0_mps(priv, mps0);
			}
			xhci_ep0_verify_mps_matches(priv, mps0,
		...
		if (xfer->err == 0 && has_data && dir_in && xfer->buf != NULL &&
			...
			if (er != 0) {
				LOG_WRN("Evaluate Context after GET_DESCRIPTOR(18) failed: %d",
					er);
			}
		}
		...
		if (priv->bulk_active_xfer[dci] != NULL) {
			LOG_ERR("bulk DCI%u: xfer already active (ep=0x%02x)",
				(unsigned int)dci, xfer->ep);
			xfer->err = -EBUSY;
			uhc_xfer_return(dev, xfer, xfer->err);
			return 0;
		}
		...
		if (xhci_dwc3_bulk_queue_prologue(priv, dci) != 0) {
			LOG_ERR("bulk DCI%u: queue prologue failed (ep=0x%02x)",
				(unsigned int)dci, xfer->ep);
			xfer->err = -EIO;
			uhc_xfer_return(dev, xfer, xfer->err);
			return 0;
...

		{
			...
			if (ep_st != XHCI_EP_CTX_EP_STATE_RUNNING &&
			    xfer->udev != NULL && xfer->udev->cfg_desc != NULL) {
				...
				if (force_drop_add &&
				    xhci_bulk_eps_reconfigure_drop_add(priv, xfer->udev, true) == 0) {
			...
			if ((ap & 63ULL) != 0ULL) {
				if (trb_len > (uint32_t)CONFIG_UHC_DWC3_BULK_OUT_STAGING_BUFSZ) {
					LOG_ERR("bulk OUT staging: trb_len %u > STAGING_BUFSZ %u",
						...
						(unsigned int)CONFIG_UHC_DWC3_BULK_OUT_STAGING_BUFSZ);
					xfer->err = -E2BIG;
					uhc_xfer_return(dev, xfer, xfer->err);
					return 0;
				}
				...
				UHC_DWC3_DBG("bulk OUT 64-byte DMA staging (was phys 0x%016llx len=%u)",
		...
		if (dir_in) {
			 ...
			 */
			const bool bulk_in_force_staging = xhci_plat_quirk(
				&cfg->xhci_plat, XHCI_QUIRK_BULK_IN_STAGING);
		...
		uint32_t trbs_needed = xhci_bulk_count_trbs_for_td(out_dma_ptr, trb_len,
								   ep_mps_sz, dir_in);
...

		{
			...
			while (remaining > 0U) {
				uint32_t chunk = xhci_bulk_next_chunk_len(out_dma_ptr, run, remaining,
									  ep_mps_sz, dir_in);
				...
				if (xfer->buf && !dir_in && chunk > 0U) {
					dwc3_dma_flush_aligned((uint8_t *)out_dma_ptr + (run - chunk), chunk);
				}
...

				{
					...
					snprintk(ctx, sizeof(ctx), "bulk %s #%u chunk=%u rem_td=%u",
						 dir_in ? "IN" : "OUT", idx,
						 (unsigned int)chunk, (unsigned int)remaining);
					...
					if (trbs_needed > 1U && dir_in && (idx == 0U || !more)) {
						UHC_DWC3_DBG("bulk IN multi-TRB TRB%u: len=%u "
							...
							(unsigned int)td_rem,
							(t->control & XHCI_TRB_CHAIN) ? 1U : 0U,
 ...
 */
static int uhc_dwc3_add_endpoints(const struct device *dev,
				    struct usb_device *const udev)
{
	...
	UHC_DWC3_DBG("device configured: addr=%u speed=%u cfg=%u",
		udev->addr, (unsigned int)udev->speed, cfg->bConfigurationValue);
	...
	for (unsigned int n = 1U; n < 16U; n++) {
		...
		if (epd != NULL) {
			UHC_DWC3_DBG("  ep OUT idx %u addr 0x%02x type %u mps %u mult %u interval %u",
				n, epd->bEndpointAddress,
		...
		if (epd != NULL) {
			UHC_DWC3_DBG("  ep IN  idx %u addr 0x%02x type %u mps %u mult %u interval %u",
				n, epd->bEndpointAddress,
				(unsigned int)(epd->bmAttributes & USB_EP_TRANSFER_TYPE_MASK),
				(unsigned int)USB_MPS_EP_SIZE(epd->wMaxPacketSize),
				(unsigned int)USB_MPS_ADDITIONAL_TRANSACTIONS(epd->wMaxPacketSize),
 ...
 */
int xhci_cancel_ep_xfer(struct uhc_dwc3_data *priv, uint8_t dci,
			       struct uhc_transfer *xfer, int err)
{
	...
	if (stop_ret != 0) {
		LOG_WRN("cancel DCI%u Stop EP failed %d — continue cleanup",
			(unsigned int)dci, stop_ret);
	}
...
mm_reg_t uhc_dwc3_dwc3_base_get(struct uhc_dwc3_data *priv)
{
	...
	cfg = priv->dev->config;
	return cfg != NULL ? cfg->dwc3_base : 0U;
}
...
int uhc_dwc3_xhci_set_tr_dequeue(struct uhc_dwc3_data *priv, uint32_t ep_index,
				 uint64_t deq)
{
...
#define UHC_DWC3_DEVICE_DEFINE(n)						\
	...
	{									\
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority),		\
			    uhc_dwc3_isr, DEVICE_DT_INST_GET(n), 0);		\
	...
	static const struct uhc_dwc3_config uhc_dwc3_cfg_##n = {		\
		.dwc3_base = DT_INST_REG_ADDR_BY_IDX(n, 0),			\
		.xhci_base = DT_INST_REG_ADDR_BY_IDX(n, 0),			\
	...
	static struct uhc_data uhc_dwc3_data_##n = {				\
		...
		.priv = &uhc_dwc3_priv_##n,					\
										...
										\
	DEVICE_DT_INST_DEFINE(n, uhc_dwc3_preinit, NULL,			\
			      &uhc_dwc3_data_##n, &uhc_dwc3_cfg_##n,		\
