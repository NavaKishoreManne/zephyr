...
uint32_t xhci_in_bytes_from_event(uint32_t buf_len, uint32_t lenfield,
					 uint32_t cc)
{
	uint32_t got;
...
void xhci_dbg_log_normal_trb(const struct xhci_trb *t, const char *ctx)
{
	...
	UHC_DWC3_DBG("xHCI Normal TRB [%s]: buf=0x%016llx st=0x%08x TRB_LEN=%u TD_SIZE=%u "
		...
		"DIR_IN=%u TRT=%u",
		ctx, (unsigned long long)((uint64_t)t->param_lo | ((uint64_t)t->param_hi << 32)),
...
static void xhci_dbg_log_xfer_event_trb(const struct xhci_trb *evt)
{
	...
	UHC_DWC3_DBG("xHCI Transfer Event: COMP=%u len/resid(23:0)=0x%06x st=0x%08x "
		"TRB_ptr=0x%016llx | ctl=0x%08x TYPE=%u CY=%u ep_id=%u slot_id=%u",
		cc, rlen, st,
		(unsigned long long)((uint64_t)evt->param_lo | ((uint64_t)evt->param_hi << 32)),
		c, (unsigned int)XHCI_TRB_FIELD_TO_TYPE(c), (c & XHCI_TRB_CYCLE) ? 1U : 0U,
...
static void xhci_dbg_log_cmd_completion_evt(const struct xhci_trb *evt)
{
	...
	UHC_DWC3_DBG("xHCI Command Completion: COMP=%u st=0x%08x cmd_trb=0x%016llx | "
...
void xhci_bulk_giveback_urb(struct uhc_dwc3_data *priv, uint8_t dci, int br,
				   uint32_t cc, uint32_t lenfield)
{
...
static void xhci_log_cmd_completion_trb(struct uhc_dwc3_data *priv, uint64_t trb_phys,
					uint32_t cc, uint32_t evt_slot_id)
{
	...
	if (cr->trbs == NULL || cr->num_trbs == 0U) {
		LOG_WRN("xHCI: cmd CC=%u evt_slot=%u ptr=0x%016llx (no cmd ring)",
			(unsigned int)cc, (unsigned int)evt_slot_id,
...

	{
		...
		if (cc != XHCI_COMP_SUCCESS) {
			LOG_WRN("xHCI: cmd CC=%u completed cmd_ring[%u] ptr=0x%016llx "
				...
				(unsigned int)cc, (unsigned int)idx,
				(unsigned long long)trb_phys, (unsigned int)trb_type,
				xhci_trb_cmd_type_name(trb_type),
...
void xhci_handle_event(struct uhc_dwc3_data *priv, struct xhci_trb *evt)
{
	...
	case XHCI_TRB_COMMAND_COMPLETION: {
		...
		xhci_log_cmd_completion_trb(priv, cmd_ptr, priv->cmd_comp_code,
					    priv->cmd_slot_id);
	...
	case XHCI_TRB_TRANSFER_EVENT: {
		...
		if (ep_id_evt == 0U || ep_id_evt >= ARRAY_SIZE(priv->bulk_active_xfer)) {
			LOG_WRN("xfer event bad ep_id=%u (slot=%u cc=%u)",
		...
		if (ep_id_evt == (uint32_t)XHCI_DCI_DEFAULT_CONTROL) {
			...
			if (cc != XHCI_COMP_SUCCESS && cc != XHCI_COMP_SHORT_PACKET) {
				UHC_DWC3_DBG("xfer TRB FAIL raw status=0x%08x p_lo=0x%08x p_hi=0x%08x "
				...
				UHC_DWC3_DBG("xHCI: xfer TRB raw status=0x%08x p_lo=0x%08x ctl=0x%08x",
					evt->status, evt->param_lo, evt->control);
			}
			UHC_DWC3_DBG("xHCI: event TRB xfer EP0 COMP=%u lenfield=%u",
				cc, priv->xfer_length);
			if (priv->xfer_result != 0) {
				LOG_ERR("xHCI: EP0 transfer failed COMP=%u residual=%u",
					cc, priv->xfer_length);
			}
			...
			uint64_t exp = priv->bulk_expect_ioc_trb_phys[ep_id_evt];
			const bool ptr_mismatch =
				(exp != 0ULL && evt_trb_ptr != exp);
			 ...
			 *
			 * Single-TRB TDs (typical BOT CSW IN): the packet is a short transfer vs MPS,
			 * so completion is often COMP_SHORT_PACKET; integrated DWC3+xHCI sometimes
			 * reports Param != IOC TRB physical address. Dropping those events leaves
			 * bulk completion stuck until timeout (-116) — especially after a preceding
			 * multi-TRB READ10 DATA phase.
			 *
			 * Transaction errors, STALL, etc. still complete (pointer may be the
			 * failing TRB, not the IOC TRB).
			 */
			if (xhci_bulk_event_is_interior(&priv->bulk_urb[ep_id_evt].td,
							     evt_trb_ptr, cc, exp)) {
			...
			if (cc == XHCI_COMP_SUCCESS) {
				if (ptr_mismatch) {
					if (priv->bulk_td_trb_count[ep_id_evt] <= 1U) {
						/* Single-TRB TD (or unset): tolerate Param mismatch vs IOC addr */
						br = 0;
					} else {
						/*
						 * Chained bulk IN: DWC3+xHCI may report Param that does not match
						 * xhci_dma_addr(last_trb); IOC retire uses bulk_expect_ioc_trb_phys.
						 */
						br = 0;
					}
				} else {
				...
				/*
				 * IOC TRB only: interior SHORT_PACKET matched ptr_mismatch above and was
				 * skipped (xHCI treats interim SP like SUCCESS). Any SHORT_PACKET
				 * reaching here is the IOC completion for this TD — admit success.
				 * Short bulk IN (got < TD length) is normal for SCSI variable-length
				 * DATA-IN (e.g. MODE SENSE); the MSC host uses CSW residue, not exact URB
				 * length.
				 */
				br = 0;
			} else if (cc == XHCI_COMP_STALL_ERROR) {
			...
			if (br == 0) {
				UHC_DWC3_DBG("xHCI: bulk xfer TRB DCI=%u raw status=0x%08x p_lo=0x%08x "
					"COMP=%u",
					(unsigned int)ep_id_evt, evt->status, evt->param_lo,
					(unsigned int)cc);
			} else if (cc == XHCI_COMP_STALL_ERROR) {
				/*
				 * STALL is a valid protocol response (e.g. gadget ends DATA short then
				 * STALLs the STATUS read); host clears ENDPOINT_HALT and retries (USB 2.0
				 * §8.5.4 / BOT). Not a controller fault — avoid LOG_ERR noise.
				 */
			...
			UHC_DWC3_DBG("xHCI: event TRB bulk DCI=%u COMP=%u lenfield=%u",
				(unsigned int)ep_id_evt, cc, priv->bulk_xfer_length[ep_id_evt]);
			if (br != 0) {
				if (cc == XHCI_COMP_STALL_ERROR) {
					LOG_WRN("xHCI: bulk endpoint STALL (COMP=%u STALL_ERROR) DCI=%u "
						"residual=%u — protocol STALL; host CLEAR_FEATURE(ENDPOINT_HALT)",
						(unsigned int)cc, (unsigned int)ep_id_evt,
						priv->bulk_xfer_length[ep_id_evt]);
				} else {
					LOG_ERR("xHCI: bulk transfer failed DCI=%u COMP=%u residual=%u",
	...
	case XHCI_TRB_PORT_STATUS: {
		...
		LOG_INF("xHCI: port %u status change (TRB param_lo=0x%08x)",
			port_id, evt->param_lo);

		{
			const uint32_t poff = XHCI_OP_PORTSC_BASE +
					      (port_id - 1U) * XHCI_PORT_REGS_STRIDE;
...
static bool xhci_evt_unhandled(struct xhci_ring *ring)
{
	...
	return ((trb->control & XHCI_TRB_CYCLE) != 0U) ==
	       ((ring->cycle_state & 1U) != 0U);
}
...
int xhci_wait_cmd_sem(struct uhc_dwc3_data *priv, bool drain_evt_ring)
{
	...
	if (priv->cmd_comp_code != XHCI_COMP_SUCCESS) {
		if (priv->cmd_comp_code == XHCI_COMP_PARAMETER_ERROR) {
		...
		} else if (priv->cmd_comp_code ==
			   XHCI_COMP_USB_TRANSACTION_ERROR) {
			...
			LOG_ERR("xHCI: command failed, code=%u",
				priv->cmd_comp_code);
		}
		return -EIO;
	}
...
{
	struct uhc_dwc3_data *priv = CONTAINER_OF(work, struct uhc_dwc3_data,
						  event_work);
...
void uhc_dwc3_isr(const struct device *dev)
{
	...
	UHC_DWC3_DBG("ISR: IMAN=0x%08x IE=%d IP=%d", iman, !!(iman & BIT(1)),
		!!(iman & BIT(0)));
