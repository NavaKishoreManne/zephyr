...
int dwc3_host_burst_init(const struct uhc_dwc3_config *cfg)
{
	...
	dwc3_xilinx_pre_host_burst(&cfg->xilinx);
	...
	dwc3_writel(base, DWC3_GUSB3PIPECTL(0),
		    dwc3_readl(base, DWC3_GUSB3PIPECTL(0)) |
		    DWC3_GUSB3PIPECTL_PHYSOFTRST);
	dwc3_writel(base, DWC3_GUSB2PHYCFG(0),
		    dwc3_readl(base, DWC3_GUSB2PHYCFG(0)) |
	...
	timeout = 1000;
	while ((xhci_readl(op_temp, XHCI_OP_USBCMD) & XHCI_USBCMD_HCRST) &&
	       timeout > 0) {
		k_busy_wait(100);
		timeout--;
	}
	...
	dwc3_xilinx_host_tune_post(&cfg->xilinx, base);
