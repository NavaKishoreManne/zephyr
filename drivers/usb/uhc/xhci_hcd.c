...
static int xhci_op_reset_full(mm_reg_t op_base)
{
	...
	timeout = 2000;
	while ((xhci_readl(op_base, XHCI_OP_USBSTS) & XHCI_USBSTS_CNR) &&
	       timeout > 0) {
		k_busy_wait(100);
		timeout--;
	}
...
int xhci_reset(struct uhc_dwc3_data *priv, mm_reg_t xhci_base)
{
	...
	LOG_DBG("xHCI version: 0x%04x, caplength: %u",
		XHCI_HCIVERSION(cap), caplength);
	...
	LOG_DBG("xHCI: max_slots=%u max_ports=%u scratchpads=%u ctx_bytes=%u",
		priv->max_slots, priv->max_ports, priv->max_scratchpad, priv->ctx_bytes);
...
int xhci_setup(struct uhc_dwc3_data *priv)
{
	 ...
	 */
	{
		...
		cfg = (cfg & ~XHCI_CONFIG_SLOTS_MASK) |
		      (priv->max_slots & XHCI_CONFIG_SLOTS_MASK);
	...
	if (priv->max_scratchpad > 0U) {
		if (priv->max_scratchpad > XHCI_MAX_SCRATCHPADS) {
			LOG_ERR("xHCI: need %u scratchpads (driver max %u)",
				priv->max_scratchpad, XHCI_MAX_SCRATCHPADS);
			return -ENOTSUP;
		}

		for (uint32_t i = 0; i < priv->max_scratchpad; i++) {
			priv->scratchpad_table[i] =
				xhci_dma_addr(&priv->scratchpad_bufs[i][0]);
		}

		dwc3_dma_flush(priv->scratchpad_table,
			       sizeof(uint64_t) * priv->max_scratchpad);
		dwc3_dma_flush(priv->scratchpad_bufs,
			       4096U * priv->max_scratchpad);
	...
	xhci_ring_init(&priv->cmd_ring, priv->cmd_trbs, XHCI_CMD_RING_SIZE, 0U);
	{
		...
		crcr = (crcr & XHCI_CRCR_PRESERVE_MASK) |
		       (addr & ~XHCI_CRCR_PRESERVE_MASK) |
...
int xhci_start(struct uhc_dwc3_data *priv)
{
	...
	timeout = 10000;
	while (timeout > 0) {
		if ((xhci_readl(priv->op_base, XHCI_OP_USBSTS) &
		     XHCI_USBSTS_HCH) == 0U) {
			break;
		}
		k_busy_wait(10);
					    ...
					    const struct device *dev)
{
	...
	if (probe_ports > priv->max_ports) {
		LOG_INF("xHCI: boot probe min %u > HC N_PORTS=%u — using %u only",
			probe_ports, priv->max_ports, priv->max_ports);
		probe_ports = priv->max_ports;
	}
	...
	(void)uhc_submit_event(priv->dev,
			       xhci_port_speed_to_connect_event(priv->port_speed),
			       0);
