...
static int mock_scsi_exec(const struct device *dev, struct scsi_xfer *xfer)
{
	static const uint8_t inquiry_resp[36] = {
		0x00, 0x80, 0x05, 0x01, 0x1f, 0x00, 0x00, 0x00,
		0x5a, 0x45, 0x50, 0x48, 0x59, 0x52, 0x20, 0x20,
		0x4d, 0x4f, 0x43, 0x4b, 0x20, 0x44, 0x49, 0x53,
	...
	case MOCK_EXEC_SG_IO:
		if (xfer->cdb[0] == 0xABU && xfer->dir == SCSI_DATA_READ &&
		    xfer->data != NULL && xfer->data_len >= 4U) {
...
ZTEST(scsi_common, test_parse_capacity16)
{
	...
	zassert_equal(scsi_parse_read_capacity_16(cap16, sizeof(cap16), &last_lba,
						&block_size), 0, NULL);
...
ZTEST(scsi_common, test_parse_capacity10_overflow)
{
	...
	zassert_equal(scsi_parse_read_capacity_10(cap10, sizeof(cap10), &last_lba,
						 &block_size), 0, NULL);
