...
static int msc_large_file_fill_chunk(uint8_t *buf, size_t buflen, uint32_t byte_offset)
{
	...
	while (pos < buflen) {
		int n = snprintf((char *)&buf[pos], buflen - pos,
				 "USB MSC large file offset=%u\n", byte_offset + (uint32_t)pos);
...
int usb_host_msc_sample_write_large_text_file(void)
{
	...
	LOG_INF("msc_large: writing %u bytes (%u MiB) to %s",
		total, (unsigned int)CONFIG_USB_HOST_MSC_SAMPLE_LARGE_FILE_SIZE_MB, path);
