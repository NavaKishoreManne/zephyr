...
static void scsi_handle_unit_attention(struct scsi_device *sdev,
				       const struct scsi_sense *sense)
{
...
static int scsi_exec_simple(struct scsi_device *sdev, struct scsi_xfer *xfer)
{
	...
	if (xfer->status == SCSI_STATUS_CHECK_CONDITION) {
		if (!scsi_sense_valid(&xfer->sense)) {
			...
			if (scsi_cmd_request_sense(&sense_xfer, sense_buf,
						   sizeof(sense_buf)) == 0) {
				(void)scsi_exec(sdev, &sense_xfer);
				(void)scsi_parse_sense(&xfer->sense, sense_buf,
						       sizeof(sense_buf));
			}
		}
...
static int scsi_probe_capacity(struct scsi_device *sdev)
{
	...
	if (last_lba32 == 0xffffffffU) {
		...
		ret = scsi_parse_read_capacity_16(cap16, sizeof(cap16), &last_lba64,
						&block_size);
		if (ret != 0) {
			return ret;
		}
...
int scsi_exec(struct scsi_device *sdev, struct scsi_xfer *xfer)
{
	...
	if (sdev == NULL || xfer == NULL || sdev->api == NULL ||
	    sdev->api->exec == NULL) {
		return -EINVAL;
	}
	...
	} else if (xfer->status == SCSI_STATUS_GOOD ||
		   xfer->status == SCSI_STATUS_CONDITION_MET) {
		return 0;
	}
...
int scsi_read_16(struct scsi_device *sdev, uint64_t lba, uint32_t blocks,
...
int scsi_write_16(struct scsi_device *sdev, uint64_t lba, uint32_t blocks,
...
int scsi_io_read(struct scsi_device *sdev, uint64_t lba, uint32_t blocks,
		 void *buf, uint32_t len)
{
	if (sdev == NULL) {
		return -EINVAL;
...
int scsi_io_write(struct scsi_device *sdev, uint64_t lba, uint32_t blocks,
		  const void *buf, uint32_t len)
{
	if (sdev == NULL) {
		return -EINVAL;
...
int scsi_read_10(struct scsi_device *sdev, uint32_t lba, uint16_t blocks,
		 void *buf, uint32_t len)
{
	struct scsi_xfer xfer = {0};
	int ret;
...
int scsi_write_10(struct scsi_device *sdev, uint32_t lba, uint16_t blocks,
		  const void *buf, uint32_t len)
{
	struct scsi_xfer xfer = {0};
	int ret;
...
int scsi_mode_sense_6_params(struct scsi_device *sdev, void *buf, uint32_t len,
			     uint8_t pc, uint8_t page_code, uint8_t subpage,
...
int scsi_device_probe(struct scsi_device *sdev)
{
	...
	if (sdev->state == SCSI_DEV_READY && sdev->block_size != 0U &&
	    sdev->block_count != 0U) {
		return 0;
	}
