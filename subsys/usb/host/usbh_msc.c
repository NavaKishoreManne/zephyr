...
static int msc_scan_configuration(const struct usb_device *udev, const uint8_t *raw,
				  uint16_t total, struct usbh_msc_iface *out)
{
	...
	for (uint16_t off = 0U; off + 2U <= total;) {
		...
		if (type == USB_DESC_INTERFACE && len >= sizeof(struct usb_if_descriptor)) {
			...
			if (!usbh_msc_subclass_scsi_supported(match->subclass)) {
				if (out != NULL) {
					LOG_DBG("MSC strict probe: subclass 0x%02x not UFI/8070/SCSI",
						match->subclass);
					(void)memset(out, 0, sizeof(*out));
					return -ENOTSUP;
				}
...
int usbh_msc_configuration_has_bulk(const struct usb_device *udev,
...
int usbh_device_configuration_prefers(const struct usb_device *udev,
				      const void *cfg_desc, uint16_t len)
{
...
int usbh_msc_get_max_lun(struct usb_device *udev, uint8_t iface, uint8_t *max_lun_out)
{
	...
	*max_lun_out = 0U;
	...
	err = usbh_req_setup(udev, USBH_MSC_BMREQ_GET_MAX_LUN, USBH_MSC_REQ_GET_MAX_LUN, 0,
			     iface, 1U, buf);
...
static int usbh_msc_class_probe(struct usbh_class_data *const c_data,
