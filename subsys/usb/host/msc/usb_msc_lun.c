...
static struct usb_msc_lun *usb_msc_lun_find(struct usb_device *udev, uint8_t lun)
{
	...
	for (size_t i = 0U; i < ARRAY_SIZE(usb_msc_luns); i++) {
		...
		if (entry->state != USB_MSC_LUN_FREE && entry->udev == udev &&
		    entry->lun == lun) {
			return entry;
		}
	}
...
static int usb_msc_lun_finish_transport(struct usb_msc_lun *lun, int ret, const char *op)
{
	...
	if (IS_ENABLED(CONFIG_USBH_MSC_BOT_TRANSPORT_RECOVER_ON_XFER_ERR) &&
	    ret != 0 && (ret == -EIO || ret == -ETIMEDOUT || ret == -EPIPE ||
			 ret == -ECONNRESET) &&
	    lun->uhc != NULL) {
		usbh_msc_bot_transport_recover_after_err(lun->udev, lun->uhc);
	}
...
static int usb_msc_scsi_exec(const struct device *dev, struct scsi_xfer *xfer)
{
	...
	ret = usbh_msc_bot_issue(lun->udev, &lun->msc, xfer->lun, xfer->cdb, xfer->cdb_len,
				 data_in, xfer->data_len,
				 data_in ? NULL : (const uint8_t *)xfer->data,
...
int usb_msc_scsi_bind(struct scsi_device *sdev, const struct device *uhc,
		      struct usb_device *udev, const struct usbh_msc_iface *msc,
		      uint8_t lun)
{
...
int usb_msc_scsi_with_lun(const struct device *uhc, struct usb_device *udev,
			  ...
			  usb_msc_scsi_lun_fn fn, void *ctx)
{
...
{
	const uint64_t sectors = lun->disk.sector_count != 0U ? lun->disk.sector_count :
								lun->sdev.block_count;
...
int usb_msc_disk_attach(struct usb_msc_lun *lun, const struct device *uhc,
			struct usb_device *udev, const struct usbh_msc_iface *msc,
			uint8_t lun_id, const char *disk_name)
{
	...
	if (ret != 0) {
		LOG_WRN("usb_msc_disk: no FAT/exFAT volume on LUN%u — exposing full LUN",
			lun_id);
		lba_offset = 0U;
		sector_count = 0U;
	}
