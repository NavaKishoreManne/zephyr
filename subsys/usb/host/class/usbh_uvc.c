					...
					const uint8_t iface)
{
	...
	for (; if_desc != NULL; if_desc = (const void *)usbh_desc_get_next_alt_setting(if_desc, cfg_end)) {
			 ...
			 uint32_t *found_bandwidth, const void *cfg_end)
{
	...
	while ((desc = usbh_desc_get_next(desc, cfg_end)) != NULL && ep_count < if_desc->bNumEndpoints) {
