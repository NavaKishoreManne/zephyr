...
static struct usbh_hub_data hub_data;
				...
				const uint8_t iface)
{
	...
	hub_data.hub = udev;
	hub_data.status_ep = status_ep;
	hub_data.status_interval = interval;
	k_work_init(&hub_data.event_work, hub_event_handler);

	err = hub_read_descriptor(&hub_data);
	if (err != 0) {
		LOG_ERR("Failed to read hub descriptor: %d", err);
		return err;
	}

	err = hub_power_on_ports(&hub_data);
	if (err != 0) {
		LOG_WRN("Hub port power failed: %d", err);
	}

	hub_data.active = true;

	err = hub_submit_status_xfer(&hub_data);
	if (err != 0) {
		LOG_ERR("Failed to start hub status polling: %d", err);
		hub_data.active = false;
		return err;
	}

	hub_scan_existing_ports(&hub_data);
...
static int usbh_hub_class_removed(struct usbh_class_data *const c_data)
{
	...
	if (!hub_data.active || hub_data.hub == NULL) {
		return 0;
	}

	hub_data.active = false;
	k_work_cancel(&hub_data.event_work);
	hub_cancel_status_xfer(&hub_data);

	ctx = hub_data.hub->ctx;
	SYS_DLIST_FOR_EACH_CONTAINER_SAFE(&ctx->udevs, child, tmp, node) {
		if (child->parent == hub_data.hub) {
			usbh_device_disconnect(child);
		}
	}
...
USBH_DEFINE_CLASS(usbh_hub_class, &usbh_hub_class_api, &hub_data, usbh_hub_class_filters);
