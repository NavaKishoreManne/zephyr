...
bool usbh_device_still_connected(const struct usb_device *udev)
{
	...
	usbh_host_lock(ctx);
	connected = (ctx->root == udev && udev->state == USB_STATE_CONFIGURED &&
		     udev->addr != 0U);
...
static int usbh_config_descriptor_index(struct usb_device *const udev,
					const uint8_t cfg_value, uint8_t *const index_out)
{
...
__weak int usbh_device_configuration_prefers(const struct usb_device *udev,
					     const void *cfg_desc, uint16_t len)
{
	ARG_UNUSED(udev);
	ARG_UNUSED(cfg_desc);
	ARG_UNUSED(len);
...
static int usbh_device_pick_configuration(struct usb_device *const udev, uint8_t *const cfg_out)
{
	...
	for (uint8_t idx = 0U; idx < udev->dev_desc.bNumConfigurations; idx++) {
...

		{
			...
			if (total > sizeof(hdr)) {
				uint8_t *const blob = k_heap_alloc(&usb_device_heap, total, K_NO_WAIT);
				...
				if (blob != NULL) {
					...
					if (err == 0) {
						*cfg_out = hdr.bConfigurationValue;
						LOG_INF("Selected configuration %u (class preferred)",
							*cfg_out);
						return 0;
					}
				} else {
					LOG_WRN("Failed to allocate %u bytes for configuration %u scan",
...
static int usbh_device_set_configuration_locked(struct usb_device *const udev, const uint8_t num)
{
	 ...
	 */
	LOG_INF("usbh: HCD device_configured — cfg=%u addr=%u speed=%u (xHCI Configure Endpoint next)",
	...
	LOG_INF("usbh: enumeration milestone — USB_STATE_CONFIGURED cfg=%u addr=%u speed=%u "
		...
		num, udev->addr, (unsigned int)udev->speed,
		(unsigned int)cfg_desc.bNumInterfaces);
	...
	if (usb2_enum_scheme) {
...

		{
			...
			for (gd = 0U; gd < gd_tries; gd++) {
				...
				LOG_WRN("usbh: GET_DESCRIPTOR/64 try %u/%u failed err=%d",
					gd + 1U, gd_tries, err);
			}
		}
		if (err) {
...

		{
			...
			for (sa = 0U; sa < sa_tries; sa++) {
				...
				LOG_WRN("usbh: assign address try %u/%u failed err=%d",
					sa + 1U, sa_tries, err);
			}
		}
		if (err) {
		...
		LOG_INF("usbh: device idVendor=0x%04x idProduct=0x%04x",
...
error:
	...
	if (err == 0 && udev->state == USB_STATE_CONFIGURED && udev->actual_cfg != 0U) {
		...
		LOG_DBG("trace: usbh skip notify err=%d state=%d actual_cfg=%u thread=%s",
			err, (int)udev->state, udev->actual_cfg, tn != NULL ? tn : "?");
	}
