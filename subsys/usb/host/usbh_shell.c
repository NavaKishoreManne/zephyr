...
static void device_list_driver_str(const struct usb_device *udev, const char *class_s,
...
{
	if (usb_msc_disk_get_volume_name(udev, 0U, buf, buflen) <= 0) {
		snprintk(buf, buflen, "-");
	}
}
#else
			   ...
			   size_t argc, char **argv)
{
	...
	SYS_DLIST_FOR_EACH_CONTAINER(&uhs_ctx->udevs, udev, node) {
		...
		shell_print(sh, "%4u %04x:%04x %5s %5s %3s %-10s %-16s %s",
			    udev->addr,
			    udev->dev_desc.idVendor, udev->dev_desc.idProduct,
			    class_s,
			    device_list_state_str(udev->state),
			    device_list_speed_str(udev->speed),
...
SHELL_STATIC_SUBCMD_SET_CREATE(device_cmds,
	SHELL_CMD_ARG(list, NULL,
		SHELL_HELP(
			"List connected USB devices (addr, VID:PID, class, topology, driver, volume)",
			""
		),
		cmd_device_list, 1, 0),
