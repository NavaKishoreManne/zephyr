...
static int msc_bulk_in_cb(struct usb_device *const udev, struct uhc_transfer *const xfer)
{
	...
	if (xfer->err == 0) {
		...
		LOG_INF("msc_flow: bulk IN completion " USBH_MSC_ERR_FMT, USBH_MSC_ERR_ARG(xfer->err));
	} else {
		LOG_WRN("msc_bot_in: err=%d (e.g. -EIO controller COMP≠success, -EPROTO short vs TD len)",
			xfer->err);
	}
						  ...
						  const char *phase, const char *ctx)
{
	...
	if (ep_out != 0U) {
		USBH_MSC_FLOW_INF("msc_flow: %s recover STALL (%s): CLEAR_FEATURE(HALT) OUT ep=0x%02x",
				  phase, ctx, ep_out);
		r = usbh_req_clear_sfs_halt(udev, ep_out);

		if (r != 0) {
			LOG_WRN("msc_bot_recover(%s): CLEAR_FEATURE(HALT) bulk OUT ep=0x%02x "
				USBH_MSC_ERR_FMT,
				phase, ep_out, USBH_MSC_ERR_ARG(r));
		}
	}

	LOG_INF("msc_bot_recover(%s): bulk IN ep=0x%02x HALT cleared (%s)",
		phase, ep_in, ctx);
	...
	if (uhc_dev != NULL) {
		...
		if (r != 0) {
			LOG_WRN("msc_bot_recover(%s): UHC bulk sync after HALT clear " USBH_MSC_ERR_FMT,
				phase, USBH_MSC_ERR_ARG(r));
			return r;
		}

		r = usbh_eps_verify_steady(udev);
		USBH_MSC_FLOW_INF("msc_flow: %s recover STALL (%s): bulk verify steady " USBH_MSC_ERR_FMT,
		...
		if (r != 0) {
			...
			r = usbh_eps_verify_steady(udev);
			USBH_MSC_FLOW_INF("msc_flow: %s recover STALL (%s): post BOT reset verify "
					  USBH_MSC_ERR_FMT,
					  phase, ctx, USBH_MSC_ERR_ARG(r));
			if (r != 0) {
				LOG_WRN("msc_bot_recover(%s): bulk still not steady after BOT reset "
					USBH_MSC_ERR_FMT,
					phase, USBH_MSC_ERR_ARG(r));
				return r;
			}
			       ...
			       const char *phase, struct net_buf **out_buf)
{
	...
	*out_buf = NULL;
	...
	LOG_DBG("msc_bot_in(%s): queue ep=0x%02x len=%u", phase, ep_in,
		(unsigned int)nbytes);
 ...
 */
static void msc_bbb_recover_after_data_fail(struct usb_device *udev,
				...
				size_t nbytes, const char *phase)
{
	...
	LOG_DBG("msc_bot_out(%s): queue ep=0x%02x len=%u", phase, ep_out,
		(unsigned int)nbytes);
...
static void msc_dbg_log_cbw_fields(const struct usb_msc_bot_cbw *cbw, const char *phase)
{
	...
	LOG_DBG("msc CBW[%s]: sig=0x%08x tag=0x%08x d_xfer_len=%u bmCBWFlags=0x%02x "
		...
		(unsigned int)(fl & 0x7fU), cbw->bCBWLUN,
		(unsigned int)(cbw->bCBWLUN & 0x0fU), (unsigned int)cbw->bCBWCBLength,
		cbw->CBWCB[0], cbw->CBWCB[1], cbw->CBWCB[2], cbw->CBWCB[3], cbw->CBWCB[4],
		cbw->CBWCB[5], cbw->CBWCB[6], cbw->CBWCB[7], cbw->CBWCB[8], cbw->CBWCB[9]);
}
...
static void msc_log_cbw_cdb_decode(const uint8_t *cdb, uint8_t cblen, char *out, size_t out_len)
{
	...
	case USB_SCSI_WRITE10:
		if (cblen >= 10U) {
			...
			(void)snprintk(out, out_len,
				       ...
				       (unsigned int)blocks, cdb[0], cdb[1], cdb[2], cdb[3],
	...
	case SCSI_OPCODE_READ_CAPACITY_10:
		...
		break;
	default:
		(void)snprintk(out, out_len, "opcode=0x%02x CDB[%u]=%02x %02x %02x %02x %02x %02x "
					       "%02x %02x %02x %02x",
...
static void msc_log_cbw_out(const struct usb_msc_bot_cbw *cbw, const char *phase)
{
	...
	LOG_INF("msc CBW[%s]: tag=0x%08x d_xfer_len=%u data_IN=%u lun=%u cblen=%u | %s",
		phase, sys_le32_to_cpu(cbw->dCBWTag), sys_le32_to_cpu(cbw->dCBWDataTransferLength),
		(unsigned int)((fl & USB_MSC_BOT_CBW_FLAG_DATA_IN) != 0U ? 1U : 0U),
				...
				uint32_t data_len, const char *phase)
{
	...
	if (data_len != 0U) {
		if (data_in && (fl & USB_MSC_BOT_CBW_FLAG_DATA_IN) == 0U) {
			LOG_ERR("msc_bbb_precheck(%s): DATA-IN phase but bmCBWFlags bit7 clear", phase);
			return -EINVAL;
		}
		if (!data_in && (fl & USB_MSC_BOT_CBW_FLAG_DATA_IN) != 0U) {
			LOG_ERR("msc_bbb_precheck(%s): DATA-OUT phase but bmCBWFlags bit7 set", phase);
			return -EINVAL;
		}
	} else if ((fl & USB_MSC_BOT_CBW_FLAG_DATA_IN) != 0U) {
		LOG_ERR("msc_bbb_precheck(%s): dCBWDataTransferLength=0 but Data-In flag set", phase);
		return -EINVAL;
	}
				...
				uint32_t data_actlen, const char *phase)
{
	...
	if (csw_tag != expect_tag) {
		LOG_ERR("msc_csw(%s): dCSWTag mismatch - expect=0x%08x CSW=0x%08x", phase, expect_tag,
			csw_tag);
		return -EIO;
	}
	...
	if (status == USB_MSC_BOT_CSW_STATUS_PHASE_ERROR) {
		LOG_ERR("msc_csw(%s): PHASE_ERR (02) - CBW_tag=0x%08x CSW_tag=0x%08x (BOT reset recovery)",
	...
	if (status == USB_MSC_BOT_CSW_STATUS_FAILED) {
		if (residue != exp_residue_good) {
			LOG_WRN("msc_csw(%s): CSW=FAILED (01) residue=%u (strict GOOD would be %u for "
				"data_len=%u act=%u)",
...
retry_full_cbw:
	...
	if (stale_full_cbw_retries > 0U) {
		LOG_DBG("msc_bbb(%s): full CBW re-issue try=%u/%u tag=0x%08x",
			phase, stale_full_cbw_retries,
			(unsigned int)MSC_BBB_STALE_CSW_FULL_CBW_RETRIES, cmd_tag);
	}
	...
	/* DATA phase */
	if (data_len != 0U) {
		USBH_MSC_FLOW_INF("msc_flow: %s DATA %s len=%u", phase,
				  data_in ? "IN" : "OUT", data_len);
		if (data_in) {
			...
			if (ret == -EPIPE) {
				...
				if (data_actlen > data_len) {
					LOG_ERR("msc_bbb(%s): DATA-IN overrun - got %u B, max %u B (CBW xfer len)",
						phase, data_actlen, data_len);
					msc_bbb_discard_data_in(udev, data_in_out);
					return -EIO;
				}

				LOG_DBG("msc_bbb(%s): DATA-IN bytes=%u (CBW xfer max=%u)", phase, data_actlen,
					data_len);
	...
	USBH_MSC_FLOW_INF("msc_flow: %s CSW IN start (stall_retries=%u)", phase,
			  csw_stall_retries);
	/*
...
int usbh_msc_bot_issue(struct usb_device *udev, const struct usbh_msc_iface *msc,
		       uint8_t lun, const uint8_t *cdb, uint8_t cdb_len, bool data_in,
		       uint32_t data_len, const uint8_t *data_out_payload, const char *phase,
...
void usbh_msc_bot_reset(struct usb_device *udev, const struct device *uhc_dev)
{
	...
	LOG_INF("msc_reset: MSC GET_MAX_LUN - max_lun=%u (%u logical unit(s))",
		msc.max_lun, (unsigned int)msc.max_lun + 1U);
	...
	LOG_DBG("msc_reset: pre MSC RESET: bulk_steady=%d steady_post_cfg=%d",
		bulk_ok_before, steady ? 1 : 0);
	LOG_DBG("msc_reset: issuing SETUP on EP0: "
		...
		msc.iface_num);
	LOG_DBG("msc_reset: wIndex MSC iface_num=%u",
		msc.iface_num);
	...
	if (err == 0) {
		...
		if (err == 0 && uhc_dev != NULL) {
			...
			if (s != 0) {
				LOG_WRN("msc_reset: UHC bulk pipe sync after HALT clear failed (%d)", s);
				err = s;
			} else {
	...
	steady = usbh_post_configure_steady(udev);
	LOG_DBG("msc_reset: post bulk steady diag=%d steady_post_cfg=%d",
		bulk_ok_after, steady ? 1 : 0);
	...
	if (!steady) {
		LOG_WRN("msc_reset: verify FAIL - controller lost post-Configure Endpoint steady flag");
	} else {
