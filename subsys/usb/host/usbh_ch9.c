...
int usbh_req_get_hcfs_port_status(struct usb_device *const hub,
				  const uint8_t port,
				  uint16_t *const portstatus,
				  uint16_t *const portchange)
{
	...
	ret = usbh_req_setup(hub,
			     hub_req_bm(USB_REQTYPE_RECIPIENT_OTHER, true),
			     USB_HCREQ_GET_STATUS, 0, port, sizeof(ps),
			     buf);
...
int usbh_req_get_hcfs_hub_status(struct usb_device *const hub,
				 uint16_t *const hubstatus,
				 uint16_t *const hubchange)
{
	...
	ret = usbh_req_setup(hub,
			     hub_req_bm(USB_REQTYPE_RECIPIENT_DEVICE, true),
			     USB_HCREQ_GET_STATUS, 0, 0, sizeof(hs),
			     buf);
...
int usbh_req_set_hcfs_port_feature(struct usb_device *const hub,
				   const uint8_t port,
				   const uint8_t feature)
{
	return usbh_req_setup(hub,
			      hub_req_bm(USB_REQTYPE_RECIPIENT_OTHER, false),
			      USB_HCREQ_SET_FEATURE, feature, port, 0,
			      NULL);
}

int usbh_req_clear_hcfs_port_feature(struct usb_device *const hub,
				     const uint8_t port,
				     const uint8_t feature)
{
	return usbh_req_setup(hub,
			      hub_req_bm(USB_REQTYPE_RECIPIENT_OTHER, false),
			      USB_HCREQ_CLEAR_FEATURE, feature, port, 0,
			      NULL);
}

int usbh_req_clear_hcfs_hub_feature(struct usb_device *const hub,
				    const uint8_t feature)
{
	return usbh_req_setup(hub,
			      hub_req_bm(USB_REQTYPE_RECIPIENT_DEVICE, false),
			      USB_HCREQ_CLEAR_FEATURE, feature, 0, 0,
			      NULL);
}

int usbh_req_desc_hub(struct usb_device *const hub,
		      const uint16_t len,
		      void *const desc)
{
	...
	ret = usbh_req_setup(hub, bmRequestType, bRequest, wValue, 0, len,
			     buf);
	if (ret == 0 && buf->len > 0U) {
...
int usbh_hub_port_reset(struct usb_device *const hub,
			const uint8_t port,
