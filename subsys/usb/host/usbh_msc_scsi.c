...
static void msc_inquiry_log_summary(const uint8_t *inq, size_t len)
{
	...
	LOG_DBG("msc INQUIRY bits: b0=0x%02x PQual=%u DevType=%u RMB=%u | b1=0x%02x "
		...
		(unsigned int)(inq[1] & 0x80U ? 1U : 0U), inq[1],
		(unsigned int)(inq[1] & 0x7fU));
	LOG_INF("msc_inquiry: qualifier/type=0x%02x  removable=%u",
		inq[0], (unsigned int)(inq[1] & 0x80U ? 1U : 0U));
	LOG_INF("msc_inquiry: vendor=\"%s\" product=\"%s\" revision=\"%s\"",
		vendor, product, rev);
}
			      ...
			      const struct device *uhc_host)
{
	...
	LOG_INF("msc MODE SENSE(6): LUN%u pc=%u page=0x%02x sub=0x%02x alloc=%u dbd=%d",
		lun, (unsigned int)(pc & 3U), page_code, subpage, (unsigned int)alloc_len,
			  ...
			  const struct device *uhc_host, const void *data)
{
	...
	LOG_DBG("WRITE10 LBA=%u blocks=%u block_size=%u len=%u", lba, blocks, block_size, total_len);
...
static void msc_bringup_log_mode_sense6(struct scsi_device *sdev)
{
	...
	USBH_MSC_FLOW_INF("msc_flow: bringup LUN%u MODE SENSE(6) " USBH_MSC_ERR_FMT,
			  sdev->lun, USBH_MSC_ERR_ARG(ms_ret));
	if (ms_ret != 0) {
		LOG_WRN("msc_storage: LUN%u MODE SENSE(6) failed: %d (non-fatal)",
			sdev->lun, ms_ret);
		return;
	}
...
again:
	...
	LOG_INF("msc_storage: LUN%u probe OK last_lba=0x%08x block_len=%u (~%llu MiB) "
		...
		(unsigned long long)(capacity_bytes / (1024U * 1024U)),
		ctx.removable, ctx.write_protected);
				     ...
				     usbh_msc_lun0_verify_fn lun0_verify)
{
	...
	USBH_MSC_FLOW_INF("msc_flow: bringup start VID=0x%04x PID=0x%04x",
	...
	LOG_INF("msc_storage: LUN(s)=%u max_lun=%u",
		(unsigned int)msc.max_lun + 1U, msc.max_lun);
